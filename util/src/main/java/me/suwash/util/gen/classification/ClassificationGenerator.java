...
public final class ClassificationGenerator {
    ...
    public static void main(final String... args) {
        ...
        // DDソースファイル名
        final String ddOutputFileName = args[2];

        // --------------------------------------------------
        // 本処理
        // --------------------------------------------------
        int errorCount = generate(inputDirPath, outputDirPath, ddOutputFileName);
     ...
     * @return 発生エラー数
     */
    public static int generate(final String inputDirPath, final String outputDirPath, final String ddOutputFileName) {
        // 対象ディレクトリの存在確認
        FileUtils.readDirCheck(inputDirPath);
        // 出力ディレクトリの存在確認
        FileUtils.writeCheck(outputDirPath + '/' + ddOutputFileName, CHARSET);
        // 出力準備
        FileUtils.setupOverwrite(outputDirPath + '/' + ddOutputFileName);
        ...
        log.info("    ・outputDir  :" + outputDirPath);
        ...
        if (inputFiles != null) {
            for (final File inputFile : inputFiles) {
                ...
                try {
                    writeClassification(config, templateClassification, inputFile, outputDirPath);
                } catch (Exception e) {
                    log.error(MessageSource.getInstance().getMessage(UtilMessageConst.ERRORHANDLE, new Object[] {
                        "replace classification",
                        "config=" + config + ", template=" + templateClassification.getName() + ", inputFile=" + inputFile + ", outputDir=" + outputDirPath
                    }), e);
                    errorCount++;
                    continue;
                }
        ...
        if (errorCount == 0) {
            ...
            final String outputFilePath = outputDirPath + SystemUtils.FILE_SEPARATOR + ddOutputFileName;
    ...
    private static void writeClassification(
        ...
        final String outputFilePath = outputDirPath + SystemUtils.FILE_SEPARATOR + outputFileName;
        final Path pathOutput = FileSystems.getDefault().getPath(outputFilePath);
